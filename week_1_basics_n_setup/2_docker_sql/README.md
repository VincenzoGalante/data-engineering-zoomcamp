...
docker run -it \
  ...
  -p 5432:5432 \
  --network=pg-network \
  --name pgdatabase \ # How pg admin will discover the database
  postgres:13
```

sudo docker run -it -e POSTGRES_USER='root' -e POSTGRES_PASSWORD='root' -e POSTGRES_DB='ny_taxi' -v /Users/vigalante/dtc-data-engineering/ny_taxi_postgres_data:/var/lib/postgresql/data -p 5432:5432 --network=pg-network --name pgdatabase postgres:13
...
sudo docker run -it -e PGADMIN_DEFAULT_EMAIL="admin@admin.com" -e PGADMIN_DEFAULT_PASSWORD="root" -p 8080:80  --network=pg-network --name pgadmin-2 dpage/pgadmin4
